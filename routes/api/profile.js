...
router.get("/notification", auth, async (req, res) => {
  try {
    const donate = await Donate.find({ touser: req.query.userid, read: false });
    let donateprofile = [];
    for (let i = 0; i < donate.length; i++) {
      donateprofile = await Profile.find({ user: donate[i].fromuser });
      donateprofile.amount = donate[i].amount;
    }
    console.log(222, donateprofile)
    res.json(donateprofile);
  } catch (err) {
    console.error(err.message);
    res.status(500).send("Server Error");
  }
