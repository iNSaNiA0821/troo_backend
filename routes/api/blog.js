...
ipfsadd = async (
  ...
  try {
    ...
    var city = "";
    if (address[0].administrativeLevels.level2long == undefined) {
      city = address[0].extra.neighborhood;
    } else {
      city = address[0].administrativeLevels.level2long;
    }
    if (!blog) {
      const newBlog = new Blog({
        ...
        address: city + ", " + address[0].country,
      ...
      if (filetype == "image") {
        let newvalue = Number(user.vcoin) + 0.1;
        user.vcoin = newvalue;
      } else {
        let newvalue = Number(user.vcoin) + 0.2;
        user.vcoin = newvalue;
      }
      await user.save();
...
takescreenshot = (file, thumbnailname) => {
  return new Promise((resolve, reject) => {
    ffmpeg(file)
      ...
      .takeScreenshots({ count: 1, timemarks: ['00:00:01.000'], filename: thumbnailname, size: '1280x720' }, "public/thumbs/");
  });
}
videomark = (file) => {
  return new Promise((resolve, reject) => {
...
router.post("/:id", auth, checkObjectId("id"), async (req, res) => {
  ...
  const user = await User.findOne({ _id: blog.creator });

  if (blog.filetype == "image") {
    let newvalue = Number(user.vcoin) - 0.1 - blog.likes.length * 0.05;
    user.vcoin = newvalue;
  } else {
    let newvalue = Number(user.vcoin) - 0.2 - blog.likes.length * 0.05;
    user.vcoin = newvalue;
  }
  await user.save();
...
router.post("/myfeed/:type/", auth, async (req, res) => {
  const AlluserInfo = await User.findOne({ _id: req.body.select_id });
  let userId = "";
  ...
  try {
    switch (type) {
      ...
      case "all":
        ...
        break;
      case "Likes":
        blogs = await Blog.find({ likes: { $elemMatch: { user: userId } }, description: { $regex: skey } }, undefined, { skip }).sort({ date: -1 });
        break;
      default:
...
router.put("/donatehistory", auth, async (req, res) => {
...
router.put("/like/:id", auth, checkObjectId("id"), async (req, res) => {
  try {
    ...
    if (blog.likes.some((like) => like.user.toString() === req.user.id)) {
      ...
      const user = await User.findOne({ _id: blog.creator });
      let newvalue = Number(user.vcoin) - 0.05;
      user.vcoin = newvalue;
      await user.save();
    } else {
      const user = await User.findOne({ _id: blog.creator });
